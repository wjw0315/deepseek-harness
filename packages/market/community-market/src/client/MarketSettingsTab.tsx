    ...
    try {
      ...
      if (!request.signal.aborted && readRequest.current === request) {
        ...
        if (!forceRefresh
          ...
          if (readRequest.current === request) applyCatalog(refreshed)
        }
      }
    } catch {
      if (readRequest.current === request) setError(t('catalogError'))
    } finally {
...
/* jscpd:ignore-start -- parallel market views (discover/installable/installed) share toolbar/meta/empty JSX; kept separate. */
...
function InstallationCard(props: {
  ...
  const disableBundleId = installation.kind === 'managed'
    && installation.status === 'active'
    ? installation.disableBundleId
    : undefined
  const enableBundleId = installation.kind === 'managed'
    && installation.status === 'disabled'
    ? installation.enableBundleId
    : undefined
  return (
...
function ItemActionModal({
  ...
  const repoUrl = value.item.repository?.url
