...
/**
 * Construction options for {@link createMarketMediaService}.
 * Every hook and limit defaults to the production value when omitted.
 * @param fetchImage - The SSRF-restricted fetcher to use.
 * @param normalizeImage - Converts a raw image into the normalized PNG body.
 * @param now - Clock source, overridable for tests.
 * @param cacheTtlMs - How long a resolved asset stays cached.
 * @param maxCachedAssets - Upper bound on resident cached images.
 * @param maxRegisteredAssets - Upper bound on registered asset records.
 * @param maxConcurrentResolutions - Upper bound on simultaneous in-flight fetches.
 * @param createAssetRef - Generates a fresh opaque asset reference.
 */
...
const VALID_MEDIA_ROLES = new Set(['plugin-icon', 'publisher-avatar'] as const)

function canonicalCandidate(candidate: MarketMediaCandidate): MarketMediaCandidate {
  if (
    !VALID_MEDIA_ROLES.has(candidate.role)
      ...
      (cause: unknown) => {
        ...
        reject(
          cause instanceof Error
            ? cause
            : new DOMException(String(cause), 'AbortError'),
        )
      },
    )
  })
...
/**
 * Create the market media service.
 * @param options - Configuration and optional test hooks.
 * @returns a service implementing the registrar and resolve lifecycle.
 */
export function createMarketMediaService(
  ...
  const evictExcessCache = () => {
    const cached = [...assets.values()]
      .filter((entry): entry is RegisteredAsset & { cachedAt: number } => entry.cachedAt !== undefined)
      .sort((left, right) => left.cachedAt - right.cachedAt)
    while (cached.length > maxCachedAssets) {
      const entry = cached.shift()
      if (entry === undefined) break
      const asset: RegisteredAsset = entry
      delete asset.cached
      delete asset.cachedAt
    }
  }
