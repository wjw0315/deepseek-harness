...
/* jscpd:ignore-start -- parallel SSRF fetcher halves share pinning/timeout plumbing; kept separate for domain checks. */
...
/** Failure categories for an SSRF-restricted market media request. */
...
/**
 * Failure of an SSRF-restricted market media request.
 * @param code - The specific rejection reason.
 */
...
/** A DNS-resolved address pinned for an SSRF-restricted media request. */
export interface MediaPinnedAddress {
  readonly address: string
  readonly family: 4 | 6
}

/** A successfully fetched market image before normalization. */
...
/**
 * Construction options for {@link createRestrictedImageFetcher}.
 * Every hook defaults to the strict built-in behavior when omitted.
 * @param syntheticProxyHostnames - Reviewed hosts allowed to resolve through a local proxy's RFC 2544 fake-IP range.
 */
...
/**
 * Validate and deduplicate a candidate's allowed redirect hostnames.
 * @param values - The raw hostnames to normalize.
 * @returns the sorted, deduplicated hostnames.
 */
...
/**
 * Validate an image URL against an exact allowed-hostname set.
 * @param value - The remote image URL to validate.
 * @param allowedHostnames - Exact reviewed hostnames allowed for the request target.
 * @returns the parsed, https-only URL.
 */
...
function assertSafeAddress(address: string, allowSyntheticProxyAddress = false): 4 | 6 {
  ...
  if (normalized === undefined) throw new MarketMediaError('blocked-address')
...
async function resolvePinnedAddress(
  ...
  if (first === undefined) throw new MarketMediaError('blocked-address')
  ...
  return new Promise((resolve, reject) => {
    ...
    const request = https.request(url, {
      ...
      void readBody(response, maxBodyBytes).then(
        ...
        (cause: unknown) =>{  finish(() =>{  reject(cause instanceof Error ? cause : new MarketMediaError('response')) }) },
      )
    })
    request.once('error', (cause) =>{  finish(() =>{  reject(cause instanceof Error ? cause : new MarketMediaError('response')) }) })
    request.once('timeout', () => request.destroy(new MarketMediaError('timeout')))
    timerState.timer = setTimeout(() => request.destroy(new MarketMediaError('timeout')), FIRST_BYTE_TIMEOUT_MS)
    request.end()
  })
}
...
/**
 * Fetch and validate a market image for a candidate.
 * @param candidate - The reviewed candidate identifying the remote image.
 * @param signal - Abort signal for the whole fetch.
 * @returns the fetched raw image.
 */
...
/**
 * Create an SSRF-restricted market image fetcher.
 * @param options - Configuration and optional test hooks.
 * @returns a fetcher that retrieves reviewed images over pinned, non-blocked addresses.
 */
export function createRestrictedImageFetcher(
  ...
  return async (candidate, signal) => {
    ...
    const onAbort = () => {
      const cause: Error = signal.reason instanceof Error
        ? signal.reason
