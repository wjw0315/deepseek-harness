...
/** Normalize a provider repository record into a credential-free HTTPS identity, removing credentials and validating its subdirectory.
 * @param repository - the provider repository record to normalize.
 * @returns the normalized repository identity, throwing a CatalogContractError when the URL or subdirectory is invalid.
 */
...
/** Normalize a provider package record into the npm registry identity it claims.
 * @param packageIdentity - the provider package record to normalize.
 * @returns the normalized package identity pinned to the npm registry.
 */
...
/** Enumerate the identities a catalog item advertises, one choice per present package or repository record.
 * @param item - the catalog item to inspect.
 * @returns a normalized package and/or repository identity per advertised record, in declaration order.
 */
