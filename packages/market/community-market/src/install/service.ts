...
/** A desktop profile that manages an installed set of DSH plugins. */
export interface MarketDesktopProfile {
  readonly name: string
  readonly dir: string
}

/** Exit status of a completed desktop pnpm invocation. */
export interface MarketDesktopPnpmOutcome {
  readonly exitCode: number | null
  readonly signal: NodeJS.Signals | null
}

/** Live streams and completion promise for an in-flight desktop pnpm invocation. */
...
/** The desktop pnpm boundary that executes and recovers plugin package operations. */
...
/** Confirmation preview for a pending plugin install operation. */
...
/** Confirmation preview for a pending plugin uninstall operation. */
...
/** Result of a completed plugin install operation. */
...
/** Union result of an executed market operation, tagged by action and carrying a restart grant. */
...
/** Machine-readable codes for failures produced by the market install workflow. */
...
/** Verifies an npm package target and yields the integrity evidence for installation. */
...
/** Integrity evidence for a verified npm package target. */
...
/** Tunables for the market install service. */
 ...
 * @param http catalog HTTP client used to fetch npm registry metadata
 * @returns verifier that proves an npm package target before installation
 */
...
export class MarketInstallService {
  ...
  /**
   * Reconcile the verified install candidates with a freshly observed catalog snapshot.
   * @param snapshot catalog snapshot whose items become candidate install targets
   */
  ...
  /**
   * Drop every candidate and install intent that originates from a given catalog source.
   * @param sourceRecordId source whose candidates and intents are discarded
   */
  ...
  /**
   * List the market install receipts owned by the active desktop profile.
   * @returns install receipts for the active profile
   */
   ...
   * @param signal abort signal that cancels the on-disk verification
   * @returns receipts whose installed bundle still matches the recorded artifact
   */
  ...
  /**
   * Produce the installable view of a catalog index with its manual install hints.
   * @param index catalog full index to reconcile against
   * @param signal abort signal that cancels the operation
   * @returns installable response with verified items and manual install hints
   */
  listInstallable(
  ...
  /**
   * Verify one catalog item and issue a short-lived install confirmation.
   * @param sourceRecordId catalog source of the item
   * @param itemId catalog item id to preview for install
   * @param signal abort signal that cancels the verification
   * @returns install preview with a confirmation intent token
   */
  ...
  /**
   * Execute a previously previewed install, verifying the package again before mutating the profile.
   * @param token confirmation intent token from previewInstall
   * @param signal abort signal that cancels the operation
   * @returns install result carrying the persisted receipt
   */
  ...
  /**
   * Execute a previewed operation (install or uninstall) and issue a restart grant.
   * @param token confirmation intent token
   * @param signal abort signal that cancels the operation
   * @returns tagged operation result with a restart token
   */
   ...
   * @param token restart grant to consume
   */
  ...
  /**
   * Verify an install receipt against the installed bundle and issue an uninstall confirmation.
   * @param receiptId install receipt to preview for uninstall
   * @param signal abort signal that cancels the verification
   * @returns uninstall preview with a confirmation intent token
   */
  ...
  /**
   * Execute a previously previewed uninstall, removing the plugin from the active profile.
   * @param token confirmation intent token from previewUninstall
   * @param signal abort signal that cancels the operation
   * @returns uninstall result identifying the removed package
   */
  ...
  /** Release all in-flight work and stored candidates, intents, and restart grants. */
  dispose(): void {
    if (this.closed) return
    this.closed = true
