 ...
 * A hint is returned only when the item has an npm registry, a stable
 * exact version, and a package name matching the npm naming pattern.
 * @param item catalog item to build a manual install hint for
 * @returns npm manual install hint, or undefined when no verifiable npm target exists
 */
...
/**
 * Build the manual install hints for every catalog item that yields one.
 * @param items catalog items to consider
 * @returns all non-undefined manual install hints, one per installable item
 */
