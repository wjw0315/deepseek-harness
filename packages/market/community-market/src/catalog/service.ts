...
/**
 * Static description of a built-in cooperating catalog provider, used to
 * present and attribute a source that ships with the community market.
 */
...
/**
 * The set of built-in cooperating catalog providers available for users to
 * explicitly add and enable as sources.
 */
...
/**
 * Catalog operations over the active local source set: list sources, run live
 * provider fetches, scan and query a full-source catalog index, and invalidate
 * a source's cached state.
 */
...
/**
 * Options that control a full-source catalog scan.
 */
...
/**
 * Construction options for DefaultCatalogService, letting a caller bound cache
 * sizes and TTLs, supply adapter HTTP clients or a media registry, and observe
 * validated host snapshots.
 */
...
/**
 * Default CatalogService implementation: loads the active source, fetches
 * provider pages under adapter and concurrency gates, caches a validated
 * full-source index, and serves paginated catalog queries with scoped cursors.
 */
export class DefaultCatalogService implements CatalogService {
  ...
  /**
   * Filter, sort, and paginate a full-source catalog index against the given
   * query, applying an explicit source scope and cursor when provided.
   * @param index - the full-source index to query.
   * @param value - the raw catalog query to normalize and apply.
   * @param scope - the active source scope, with an optional cursor token.
   * @returns the matching, page-sliced catalog results for the active source.
   */
  queryCatalog(
    index: CatalogFullIndex,
    value: unknown,
  ...
  /**
   * Ensure a fresh full-source index and query it for the given catalog query,
   * optionally scoped to a specific source.
   * @param value - the raw catalog query to normalize and apply.
   * @param signal - the abort signal that cancels the underlying scan.
   * @param scope - the active source scope to fetch for.
   * @returns the matching catalog results, or an empty list when no source is
   * active.
   */
  async fetch(
    value: unknown,
