...
/**
 * Failure category for an SSRF-restricted catalog HTTP request.
...
/** A DNS-resolved address pinned for an SSRF-restricted request. */
...
/**
 * Construction options for {@link createRestrictedHttpClient}.
 * Every hook defaults to the strict built-in behavior when omitted.
 * @param syntheticProxyHostnames - Reviewed hostnames allowed to resolve through a local proxy's RFC 2544 fake-IP range.
 */
...
function assertSafeAddress(address: string, allowSyntheticProxyAddress = false): 4 | 6 {
  ...
  if (normalized === undefined) throw new CatalogNetworkError('blocked-address')
...
/**
 * Shape the pinned address for a `dns.lookup` callback.
 * @param options - The DSN lookup options passed by Node's https request.
 * @param pinned - The pinned address to return.
 * @returns a single address, or an array when `options.all` is set.
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    const request = https.request(url, {
      ...
      void readBody(response, maxBodyBytes).then(
        ...
        (cause: unknown) =>{  finish(() =>{  reject(cause instanceof Error ? cause : new CatalogNetworkError('response')) }) },
      )
    })
...
/**
 * Create an SSRF-restricted catalog HTTP client.
 * @param options - Configuration and optional test hooks.
 * @returns a client that fetches JSON over pinned, non-blocked addresses.
 */
...
/**
 * Construction options for {@link createCachedCatalogHttpClient}.
 * @param ttlMs - How long a completed response stays fresh.
 * @param now - Clock source, overridable for tests.
 */
 ...
 * @param delegate - The underlying restricted client to cache results for.
 * @param options - TTL and clock configuration.
 * @returns a caching wrapper around the delegate client.
 */
...
/** The shared default SSRF-restricted catalog HTTP client. */
