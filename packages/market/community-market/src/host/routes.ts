...
/** Settings namespace key under which the community-market plugin stores its sources, install receipts, and catalog cache. */
...
/** Context describing an inbound HTTP request to the market surface, used to authorize it as a local request. */
...
/** Reports whether the request described by the context is a local, loopback request for the market surface.
 * @param context describes the inbound request to authorize.
 * @returns true when the request originates from the local loopback on the expected port.
 */
export function marketRequestAllowed(context: MarketRequestContext): boolean {
  return marketAuthority(context) !== undefined
}

/** Reports whether the request is a local same-origin request allowed to mutate market state.
 * @param context describes the inbound request to authorize.
 * @returns true when the request is both loopback-local and same-origin with the market host.
 */
...
/** Provider resolving the active market install service, or undefined when package operations are unavailable. */
export interface MarketInstallServiceProvider {
  get(): MarketInstallService | undefined
}

/** Desktop host actions the market surface can trigger on request. */
export interface MarketDesktopActions {
  openTerminal(): void
  requestRestart(): Promise<void>
}

/** Provider resolving the active desktop actions, or undefined when the desktop host is unavailable. */
export interface MarketDesktopActionsProvider {
  get(): MarketDesktopActions | undefined
}

/** A desktop plugin bundle as reported by the desktop plugin inventory. */
...
/** A confirmed preview of disabling a desktop plugin bundle. */
...
/** A confirmed preview of enabling a desktop plugin bundle. */
...
/** Desktop plugin inventory and enable/disable operations exposed to the market surface. */
...
/** Provider resolving the active desktop plugins surface, or undefined when plugin management is unavailable. */
...
/** Fetches and validates a standard source manifest over credential-free HTTPS port 443.
 * @param manifestUrl the HTTPS manifest URL to load.
 * @param signal aborts the fetch when triggered.
 * @param http the restricted HTTP client used for the fetch.
 * @returns the parsed and trust-checked catalog source manifest.
 */
...
async function mutateSources(
  ...
  if (mutation.action === 'add-builtin') {
    ...
    if (mutation.action === 'remove') {
      ...
      if (record === undefined) throw new Error('source not found')
    ...
    if (current === undefined || target === undefined) throw new Error('source not found')
...
/** Creates a serialized source mutation runner that applies each mutation only after the previous one settles.
 * @param scope the settings scope holding the market source records.
 * @param onUnavailable optional callback invoked with source record ids that became unavailable.
 * @param readManifest optional manifest loader, defaulting to {@link readStandardSourceManifest}.
 * @returns a function that enqueues and applies a source mutation with the given abort signal.
 */
...
/** Registers the community-market HTTP routes on the local web server and returns a disposer.
 * @param ctx the plugin context providing the web server.
 * @param scope the market settings scope the routes read and mutate.
 * @param installProvider optional provider of the market install service.
 * @param desktopActionsProvider optional provider of desktop host actions.
 * @param desktopPluginsProvider optional provider of desktop plugin management.
 * @returns a disposer that aborts in-flight work and unregisters all routes.
 */
  ...
  const routes = [
    ctx.webServer.register({ kind: 'exact', path: ROUTE_STATE, handler: async (_req, res) => {
      ...
      try {
        ...
        if (!res.destroyed) sendJson(res, 200, response)
      } catch {
        if (!res.destroyed) sendJson(res, 500, { error: 'market state unavailable' })
      }
    } }),
      ...
      ctx.webServer.register({ kind: 'exact', path: ROUTE_OPERATION_EXECUTE, handler: async (req, res) => {
        ...
        try {
          ...
          if (disablePreview !== undefined || enablePreview !== undefined) {
            ...
            if (desktopPreview === undefined) throw new MarketInstallError('operation-failed', 'Desktop preview not found')
...
/** Registers the market settings namespace on the context and returns its live settings scope.
 * @param ctx the plugin context on which to register the settings namespace.
 * @returns the live settings scope for the community-market settings document.
 */
...
/** Maps route names to the registered market HTTP route paths. */
