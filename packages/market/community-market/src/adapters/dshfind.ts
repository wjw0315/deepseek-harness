...
/** Registration key identifying this built-in dshfind catalog provider. */
export const DSHFIND_KEY = 'dshfind'
/** HTTPS endpoint that serves dshfind's paginated plugin catalog. */
export const DSHFIND_ENDPOINT = 'https://api.dshfind.com/v1/plugins'
/** Hostname that dshfind responses and redirects must stay on. */
export const DSHFIND_HOSTNAME = 'api.dshfind.com'
/** Stable provider identity for recorded dshfind provenance. */
export const DSHFIND_PROVIDER_ID = 'com.dshfind.catalog'
/** Adapter identity used when a dshfind source registers a session. */
...
/** Tunables for a dshfind catalog adapter instance. */
...
function repositoryFromItem(raw: Record<string, unknown>): {
  ...
  try {
    ...
    if (
      owner === undefined || repositoryName === undefined
...
/**
 * Build a dshfind catalog adapter that paginates the reviewed provider.
 * @param options - overrides for inter-page pacing and the dataset clock.
 * @returns a CatalogAdapter bound to 1024Store-style dshfind pagination.
 */
...
/** Default dshfind catalog adapter used for the anonymous production client. */
