...
/**
 * Validate that a registered standard source's manifest and transport endpoint
 * stay on one credential-free HTTPS origin and return that origin.
 * @param manifestUrlValue - the source manifest URL recorded at registration.
 * @param manifestFinalUrl - the resolved URL of the fetched manifest response.
 * @param endpointValue - the transport endpoint declared by the manifest.
 * @returns the validated origin shared by the manifest and its endpoint.
 */
...
/** Catalog adapter that fetches a registered standard catalog source over HTTPS. */
