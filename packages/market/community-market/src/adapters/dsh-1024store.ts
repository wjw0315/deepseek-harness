...
import { buildSnapshotPages } from './snapshot-pages.js'

/** Registration key identifying this built-in 1024Store catalog provider. */
export const DSH_1024STORE_KEY = 'dsh-1024store'
/** HTTPS endpoint that returns the reviewed 1024Store plugin catalog payload. */
export const DSH_1024STORE_ENDPOINT = 'https://deepseek1024.com/api/v1/plugins'
/** Hostname that media and repository assets for 1024Store must stay on. */
export const DSH_1024STORE_HOSTNAME = 'deepseek1024.com'
/** Stable provider identity for recorded 1024Store provenance. */
export const DSH_1024STORE_PROVIDER_ID = 'com.deepseek1024.catalog'
/** Adapter identity used when a 1024Store source registers a session. */
export const DSH_1024STORE_ADAPTER_ID = 'market.dsh-1024store-v1'

/** Untrusted field-level view of one raw 1024Store catalog entry. */
...
/**
 * Extract the optional provider-reported generation facts from a raw entry.
 * @param rawMeta - The untrusted raw metadata field to read.
 * @returns the normalized generation time and revision, when parseable.
 */
function resolveProviderFacts(
  ...
  return { providerGeneratedAt, providerRevision }
}
...
function buildSnapshot(value: unknown, context: CatalogFetchContext, finalUrl: string, query: CatalogQuery): CatalogSnapshot {
  ...
  const { providerGeneratedAt, providerRevision } = resolveProviderFacts(raw.meta)
...
function buildCatalogScanSnapshots(
  ...
  return buildSnapshotPages(context, items, {
    finalUrl,
    fetchedAt,
    pageSize: 100,
    providerGeneratedAt,
...
/** Catalog adapter that fetches and normalizes the reviewed 1024Store plugin catalog. */
