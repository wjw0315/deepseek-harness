...
describe('package integration contract', () => {
  it('loads the Client through the official settings, sidebar, and shell features', () => {
    ...
    // The Client composes through these official UI feature packages; this
    // workspace declares them as runtime dependencies (workspace:^).
    expect(manifest.dependencies).toHaveProperty('@deepseek-ai/dsh-client-ui-layout')
    expect(manifest.dependencies).toHaveProperty('@deepseek-ai/dsh-client-ui-settings')
    expect(manifest.dependencies).toHaveProperty('@deepseek-ai/dsh-client-ui-sidebar')
  })
})
