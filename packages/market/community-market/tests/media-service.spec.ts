...
describe('market media service', () => {
  ...
  it('normalizes to a metadata-free 128px PNG and collapses concurrent reads', async () => {
    ...
    }).jpeg().withExif({ IFD0: { Copyright: 'must be removed' } }).toBuffer()
    ...
    expect(one).toMatchObject({ contentType: 'image/png', etag: expect.stringMatching(/^"sha256-/u) as string })
...
describe('restricted market image fetcher', () => {
  ...
  it('enforces the total deadline while DNS resolution remains pending', async () => {
    ...
    try {
      ...
      // The ported fetcher checks the abort signal only before DNS resolution,
      // so a lookup that resolves after the deadline still reaches request()
      // with the already-aborted signal.
