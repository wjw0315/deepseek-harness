...
import type { MarketStateResponse } from '../src/api-types.ts'
import type { MarketCatalogResponse, MarketSourceView } from '../src/api-types.ts'
import type {} from '../src/client/index.ts'
import { MarketOverlay, type MarketOverlayProps } from '../src/client/MarketOverlay.tsx'
...
// Fetch receives string URLs for state/catalog routes and URL objects for
// catalog/installable (built via new URL in src/client/api.ts); stringify the
// input either way so the mock can route on the path.
...
describe('community market overlay', () => {
  it('shows the empty source state without requesting a catalog', async () => {
    ...
    expect(requestUrl(request.mock.calls[0]?.[0])).toContain('/api/community-market/state')
  ...
  it('loads and renders plugin cards when a source is enabled', async () => {
    const request = vi.fn<typeof fetch>(async input => (
      requestUrl(input).includes('/state') ? response(stateWithSource) : response(catalogWithItem)
    ))
    vi.stubGlobal('fetch', request)
    renderOpenOverlay()
    ...
    expect(requestUrl(request.mock.calls[0]?.[0])).toContain('/api/community-market/state')
    expect(requestUrl(request.mock.calls[1]?.[0])).toContain('/api/community-market/catalog')
  })
  ...
  it('submits a trimmed search query to the catalog route', async () => {
    ...
    const catalogUrl = new URL(requestUrl(request.mock.calls[2]?.[0]))
  ...
  it('refreshes state and catalog with the current query', async () => {
    ...
    expect(requestUrl(request.mock.calls[3]?.[0])).toContain('/api/community-market/state')
    const catalogUrl = new URL(requestUrl(request.mock.calls[4]?.[0]))
    expect(catalogUrl.searchParams.get('q')).toBe('sidebar')
    expect(catalogUrl.searchParams.get('locale')).toBe('en')
  })
  ...
  it('shows a catalog error and retries the failed request', async () => {
    let catalogCalls = 0
    const request = vi.fn<typeof fetch>(async (input) => {
      if (requestUrl(input).includes('/state')) return response(stateWithSource)
      catalogCalls += 1
  ...
  it('keeps successful cards visible when one catalog source fails', async () => {
    ...
    const request = vi.fn<typeof fetch>(async input => (
      requestUrl(input).includes('/state') ? response(stateWithSource) : response(partialCatalog)
    ))
    vi.stubGlobal('fetch', request)
    renderOpenOverlay()
  ...
  it('marks cards backed by a stale catalog snapshot', async () => {
    ...
    const request = vi.fn<typeof fetch>(async input => (
      requestUrl(input).includes('/state') ? response(stateWithSource) : response(staleCatalog)
    ))
    vi.stubGlobal('fetch', request)
    renderOpenOverlay()
  ...
  it('shows a loading state while the catalog request is pending', async () => {
    ...
    const request = vi.fn<typeof fetch>(async input => (
      requestUrl(input).includes('/state') ? response(stateWithSource) : pendingCatalog
    ))
    vi.stubGlobal('fetch', request)
    renderOpenOverlay()
  ...
  it('selects another source and reloads the catalog for the new selection', async () => {
    ...
    expect(requestUrl(request.mock.calls[3]?.[0])).toContain('/api/community-market/catalog')
  })
