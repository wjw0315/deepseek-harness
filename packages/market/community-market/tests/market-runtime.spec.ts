...
import { dsh1024StoreAdapter, DSH_1024STORE_ADAPTER_ID, DSH_1024STORE_KEY, DSH_1024STORE_PROVIDER_ID } from '../src/adapters/dsh-1024store.ts'
import { DSHFIND_ADAPTER_ID, DSHFIND_KEY, DSHFIND_PROVIDER_ID } from '../src/adapters/dshfind.ts'
import { standardHttpAdapter } from '../src/adapters/standard-http.ts'
import { DefaultCatalogService, type CatalogFullIndex } from '../src/catalog/service.ts'
import { MemoryCatalogSourceStore, SettingsCatalogSourceStore } from '../src/catalog/source-store.ts'
...
} from '../src/network/restricted-http.ts'
...
async function requestMarketCatalog(
  ...
  try {
    await handlers.get(marketRoutes.catalog)!(
      request as unknown as EventEmitter & Record<string, unknown>,
      response as unknown as EventEmitter & Record<string, unknown>,
    )
  } finally {
    dispose()
  }
...
describe('1024Store adapter', () => {
  ...
  it('projects reviewed fields and never forwards remote install strings', async () => {
    const http = { getJson: vi.fn(async () => ({ value: rawCatalog, finalUrl: 'https://deepseek1024.com/api/v1/plugins' })) }
...
describe('source mutation boundary', () => {
  ...
  it('aborts an in-flight source mutation when the plugin generation is disposed', async () => {
    type RouteHandler = (req: EventEmitter & Record<string, unknown>, res: EventEmitter & Record<string, unknown>) => Promise<void>
    ...
    const pending = handlers.get(marketRoutes.sources)!(
      request as unknown as EventEmitter & Record<string, unknown>,
      response as unknown as EventEmitter & Record<string, unknown>,
    )
...
describe('restricted HTTP boundary', () => {
  ...
  it('caches a completed fixed-catalog response and collapses concurrent reads', async () => {
    ...
    const delegate = { getJson: vi.fn(async () => await pending) }
  ...
  it('aborts a shared fixed-catalog request after its last waiter leaves', async () => {
    ...
    const delegate = {
      getJson: vi.fn(async (_url: string, signal: AbortSignal) => await new Promise<never>((_resolve, reject) => {
  ...
  it('enforces the total deadline while DNS resolution remains pending', async () => {
    ...
    try {
      ...
      await vi.advanceTimersByTimeAsync(0)
      // The ported client checks the abort signal only before DNS resolution,
      // so a lookup that resolves after the deadline still reaches request()
      // with the already-aborted total signal.
