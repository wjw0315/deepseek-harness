...
} from '../src/adapters/dsh-1024store.ts'
import {
  DSHFIND_ADAPTER_ID,
  DSHFIND_ENDPOINT,
  DSHFIND_KEY,
  DSHFIND_PROVIDER_ID,
} from '../src/adapters/dshfind.ts'
import type { MarketSettingsDocument } from '../src/catalog/source-store.ts'
import type { CatalogSourceManifest, LocalSourceRecord } from '../src/contracts/index.ts'
import { marketRoutes, registerMarketRoutes } from '../src/host/routes.ts'
import { restrictedHttpClient } from '../src/network/restricted-http.ts'
...
describe('community market Host routes', () => {
  ...
  it('normalizes catalog query parameters and returns aggregated source results', async () => {
    ...
    try {
      ...
      expect(response.status).toBe(200)
      const body = await response.json() as { fetchedAt?: unknown; [key: string]: unknown }
      expect(body).toMatchObject({
        ...
        metadata: {
          scannedAt: expect.any(String) as unknown,
  ...
  it('removes a source and compacts the remaining source order', async () => {
    ...
    try {
      ...
      const state = await readRoute(server, marketRoutes.state)
      const body = await state.json() as { sources: readonly unknown[] }
