...
import { DSH_1024STORE_ADAPTER_ID, DSH_1024STORE_PROVIDER_ID } from '../src/adapters/dsh-1024store.ts'
import type { MarketSettingsDocument } from '../src/catalog/source-store.ts'
import type { CatalogFullIndex } from '../src/catalog/service.ts'
import type { CatalogHttpClient, CatalogSnapshot } from '../src/contracts/index.ts'
import {
...
} from '../src/install/service.ts'
import { marketRoutes, registerMarketRoutes, registerMarketSettings } from '../src/host/routes.ts'
import { manualInstallHint } from '../src/install/manual.ts'
...
describe('npm registry verification', () => {
  it('pins exact identity, repository, origin, and rejects lifecycle/deprecated metadata', async () => {
    const getJson = vi.fn<CatalogHttpClient['getJson']>(async () => ({
    ...
    const verifier = createNpmRegistryVerifier({ getJson })
...
describe('market install service', () => {
  ...
  it('uses one-shot opaque intents, fixed argv, verified bundle state, and profile receipts', async () => {
    ...
    expect(calls[0]).toMatchObject({
      args: ['add', '--save-exact', '--config.minimumReleaseAge=0', '--registry=https://registry.npmjs.org/', `${packageName}@${version}`],
      dir: profileDir,
    })
  ...
  it('honors cancellation and disposal before local installable filtering', async () => {
    ...
    // listInstallable is synchronous through its abort/open guards, so a
    // pre-aborted signal and a disposed service surface as synchronous throws.
    expect(() => service.listInstallable(fullIndex(snapshot()), controller.signal))
      .toThrow(expect.objectContaining({ name: 'AbortError' }))
    service.dispose()
    expect(() => service.listInstallable(fullIndex(snapshot()), new AbortController().signal))
      .toThrow(expect.objectContaining({ code: 'operation-failed' }))
  })
  ...
  it('refuses changed profile state and nonzero package-manager outcomes without issuing receipts', async () => {
    ...
    expect(settings.receipts()).toEqual([])
    expect((JSON.parse(await readFile(join(profileDir, 'package.json'), 'utf8')) as Record<string, unknown>).dependencies).toEqual({})
  })
  ...
  it('rolls back a direct dependency written before a nonzero add outcome', async () => {
    ...
    service.observeCatalog(snapshot())
    const preview = await service.previewInstall('source-1', 'example/dsh-plugin-safe', new AbortController().signal)
    const rollbackMessage: unknown = expect.stringContaining('partial installation was rolled back')
    await expect(service.executeInstall(preview.intent, new AbortController().signal)).rejects.toMatchObject({
      code: 'operation-failed',
  ...
  it('uses an independent cleanup signal when disposal follows a partial add', async () => {
    ...
    const calls: Array<{ readonly verb: string; readonly aborted: boolean }> = []
    const settings = memoryScope()
    const serviceBox: { current: MarketInstallService | undefined } = { current: undefined }
    const pnpm = recoverableRunner(profileDir, {
      runPlugin(args, _dir, signal) {
        calls.push({ verb: args[0]!, aborted: signal?.aborted ?? false })
        const done = (async () => {
  ...
  it('does not issue an install intent when disposed during registry verification', async () => {
    ...
    const service = new MarketInstallService(
      ...
      runner(profileDir, []),
      {
        verify: vi.fn(async (_candidate: unknown, signal: AbortSignal) => {
...
describe('market install Host routes', () => {
  it('registers the optional routes and accepts only exact local operation bodies', async () => {
    ...
    const executePreview = vi.fn(async () => ({ action: 'install', receipt: { receiptId: 'receipt' }, restartToken: 'restart-token' }))
    const consumeRestartToken = vi.fn()
    const install = {
    ...
    expect(executePreview).toHaveBeenCalledWith('opaque-preview-id', expect.any(AbortSignal))
    ...
    expect(consumeRestartToken).toHaveBeenCalledWith('restart-token')
    ...
    await expect(request(marketRoutes.requestRestart, 'POST', { restartToken: disableRestartToken }))
      .resolves.toEqual({ status: 200, body: { ok: true } })
    expect(consumeRestartToken).toHaveBeenCalledTimes(1)
  ...
  it('reconciles verified receipts with direct bundles without starting catalog verification', async () => {
    ...
    const listInstallable = vi.fn()
    const listVerifiedReceipts = vi.fn(async () => [])
    const install = {
      listReceipts: vi.fn(async () => []),
      listVerifiedReceipts,
      listInstallable,
    ...
    expect(listInstallable).not.toHaveBeenCalled()
    expect(listVerifiedReceipts).toHaveBeenCalledOnce()
