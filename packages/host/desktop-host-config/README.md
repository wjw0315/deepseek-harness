...
1. It registers a settings namespace (`desktop-host-config`) through `installSettingsSection`, so the values are editable in the GUI settings and persist in the harness settings document.
...
A standalone `dsh web` (no `DSH_DESKTOP_HOST_CONFIG` env) shows the settings card but writes nothing; only the Electron shell feeds the file the desktop reads.
