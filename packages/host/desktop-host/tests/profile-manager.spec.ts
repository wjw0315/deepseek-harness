...
describe('desktop profile discovery', () => {
  it('accepts safe human profile names while rejecting path and control characters', () => {
    expect(() => {
      assertDesktopProfileName('团队 profile')
    }).not.toThrow()
    expect(() => {
      assertDesktopProfileName('profile\nname')
    }).toThrow('invalid desktop profile name')
    expect(() => {
      assertDesktopProfileName('../outside')
    }).toThrow()
  })
  ...
  it('lists lazy defaults and existing profiles without creating or changing manifests', () => {
    ...
    expect(listDesktopProfiles(home)).toEqual([
      ...
      expect.objectContaining({ name: 'broken', exists: true, webCapable: false, problem: expect.any(String) as unknown as string }),
      expect.objectContaining({
        name: 'embedded-desktop',
        exists: true,
        webCapable: false,
        problem: expect.stringContaining('composed by the web-app bundle') as unknown as string,
      }),
  ...
  it('treats an existing repairable desktop profile as managed but rejects malformed metadata', () => {
    const home = temporaryRoot()
    writeProfile(home, 'web', ['@deepseek-ai/dsh-base'])
    ...
    expect(listDesktopProfiles(home)[0]).toEqual(expect.objectContaining({
      name: 'web',
      webCapable: false,
      problem: expect.any(String) as unknown as string,
    }))
  })
})
...
describe('desktop profile selection state', () => {
  it('defaults to desktop and queues only a directly Web-capable profile', () => {
    ...
    expect(() => selectDesktopProfile(statePath, home, 'embedded-desktop')).toThrow('composed by the web-app bundle')
  ...
  it('consumes a pending profile and rolls back an unconfirmed startup on the next launch', () => {
    ...
    expect(beginDesktopProfileStartup(statePath, home)).toEqual({
      profileName: 'work',
      state: { version: 1, active: 'work', lastKnownGood: 'web' },
