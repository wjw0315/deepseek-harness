...
/** Deferred whose public surface avoids a `void` type argument (tests enable no-invalid-void-type). */
...
describe('desktop profile service', () => {
  ...
  it('lets the first concurrently persisted target win', async () => {
    const firstPersist = voidDeferred()
    const firstRestart = voidDeferred()
    const events: string[] = []
  ...
  it('retains a persisted target after restart failure and retries only that restart', async () => {
    const retryRestart = voidDeferred()
