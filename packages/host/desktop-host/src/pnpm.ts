 ...
 * Every path — including the Electron executable reused through RunAsNode —
 * is resolved and injected by the desktop launcher; this package never
 * resolves or hardcodes an Electron location itself.
 */
export interface DesktopPnpmBootstrap {
  ...
  /** Electron executable reused through RunAsNode; launcher-provided. */
...
export class DesktopPnpm extends Service {
  ...
  constructor(ctx: Context, private readonly bootstrap: DesktopPnpmBootstrap) {
    ...
    ctx.effect(
      ...
      'dsh-desktop-host: active pnpm operation teardown',
    )
  }
   ...
   * @param args - `dsh plugin add` arguments for the packaged CLI.
   * @param invokingDir - absolute caller directory used to anchor relative package specifications.
   * @param recovery - installed package identity and the host-minted receipt id.
   * @param signal - optional cancellation for this operation.
   * @returns live output streams, completion, and cancellation.
   */
  ...
  /**
   * List rolled-back installs whose receipts Market cleanup still awaits.
   * @returns the exact rolled-back receipt ids, empty when none are pending.
   */
   ...
   * @param receiptId - the Market receipt whose rolled-back transaction to clear.
   */
   ...
   * @param receiptId - the install receipt whose transaction to roll back.
   * @returns whether the receipt identified a live install transaction.
   */
