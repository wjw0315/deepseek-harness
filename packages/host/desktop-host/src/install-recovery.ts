...
/** One allowlisted declarative file whose pre-install bytes restore a profile. */
...
/** Lifecycle phase of one install transaction recorded in the WAL. */
...
/** Why a transaction reached verification, rollback, or manual recovery. */
...
/** Caller-supplied identity of the package operation a transaction guards. */
...
/** Construction inputs fixing the WAL path, profile, and owning generation. */
...
/** What the current generation must do with a pending transaction on startup. */
...
/** Outcome of restoring one transaction's pre-install file bytes. */
 ...
 * @param userDataDir - absolute Electron user-data directory owning the private state tree.
 * @param environment - environment overrides carrying the terminal hand-off.
 * @returns the absolute WAL state file path.
 */
...
export class DesktopInstallRecoveryStore {
  /** Absolute path of the fixed Desktop-private WAL file. */
  readonly statePath: string
  /** Profile name this store instance reconciles. */
  readonly profileName: string
  /** Absolute profile directory the backed-up files belong to. */
  readonly profileDir: string
  /** Opaque identity of the Electron main generation owning this store. */
  readonly generationId: string
   ...
   * @returns the stored transaction, or undefined when no WAL exists.
   */
   ...
   * @param input - installed package identity and the host-minted receipt id.
   * @returns the published prepared transaction.
   */
    ...
    try {
      for (const name of DESKTOP_INSTALL_RECOVERY_FILES) {
        ...
        if (image.metadata.present) {
          ...
          if (bytes === undefined) {
            throw new Error(`${BIN_NAME}: plugin install recovery could not read ${name}`)
          }
   ...
   * @param transactionId - the transaction whose post-install image to seal.
   * @returns the transaction advanced to awaiting-restart.
   */
   ...
   * @param transactionId - the origin generation's exclusive transaction.
   * @param failureReason - classification recorded on the rollback.
   * @returns the restore outcome keyed by status.
   */
   ...
   * @returns the startup decision for this generation.
   */
   ...
   * @param transactionId - the transaction this generation was verifying.
   * @param failureReason - why verification failed.
   * @returns the transaction moved to recovery-pending.
   */
  async recordFailure(
    transactionId: string,
    failureReason: DesktopInstallRecoveryFailureReason,
   ...
   * @param transactionId - the recovery-pending transaction to retry.
   * @returns the transaction moved to retry-requested.
   */
   ...
   * @param transactionId - the transaction this generation was verifying.
   * @returns the terminal verified transaction.
   */
   ...
   * @param transactionId - the already rolled-back transaction.
   * @returns the transaction with rollbackNotifiedAt set.
   */
  ...
  private async restoreLocked(
    ...
    for (const file of state.files) {
      ...
      if (!file.before.present) {
        ...
        const backup = backups.get(file.name)
        if (backup === undefined) {
          throw new Error(`${BIN_NAME}: plugin install recovery backup for ${file.name} is missing`)
        }
        await writeFileAtomic(target, backup.toString('utf8'), {
   ...
   * @param transactionId - the active transaction that cannot roll back its files.
   * @param failureReason - the failure that forced manual recovery.
   * @returns the terminal manual-recovery-required transaction.
   */
   ...
   * @param transactionId - the terminal transaction to clear.
   */
  async clear(transactionId: string): Promise<void> {
    await this.withMutationLock(async () => {
      await this.clearLocked(transactionId)
    })
  }
  ...
  private async readBackup(
    ...
    if (backup.bytes === undefined) {
      throw new Error(`${BIN_NAME}: plugin install recovery backup for ${file.name} is unreadable`)
    }
    return backup.bytes
  }
  ...
  private async readBoundedRegularFile(path: string, limit: number, required: boolean): Promise<ReadFileImage> {
    ...
    try {
      handle = await open(path, constants.O_RDONLY | O_NOFOLLOW_BEST_EFFORT)
