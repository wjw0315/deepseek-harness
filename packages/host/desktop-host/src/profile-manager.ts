/** Desktop Host profile discovery and restart-safe selection state. */
...
const DESKTOP_BUNDLE_NAME = '@deepseek-ai/dsh-desktop-host'
...
function existingProfile(name: string, home: string): DesktopProfileSummary {
  ...
  try {
    ...
    const problem = desktopBundleIndex !== -1
      ? `${DESKTOP_BUNDLE_NAME} is composed by the web-app bundle and must not appear in dsh.profile.bundles`
      : undefined
...
function virtualProfile(name: string, home: string): DesktopProfileSummary {
...
export function listDesktopProfiles(home: string): DesktopProfileSummary[] {
  ...
  for (const name of [DEFAULT_PROFILE_NAME] as const) {
