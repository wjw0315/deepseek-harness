...
export interface DesktopProfileServiceBootstrap {
  ...
  /** Request orderly teardown followed by a host relaunch. */
...
export class DesktopProfileService extends Service implements DesktopProfiles {
  ...
  constructor(ctx: Context, private readonly bootstrap: DesktopProfileServiceBootstrap) {
    ...
    ctx.effect(
      () => () => { this.disposed = true },
      'dsh-desktop-host: profile service lifetime',
    )
  }
  ...
  private committedSelectionError(name: string): Error {
    return new Error(
      `dsh-desktop-host: profile ${JSON.stringify(this.committedName)} is already pending; `
  ...
  private assertActive(): void {
    if (this.disposed) throw new Error('dsh-desktop-host: desktopProfiles service disposed')
  }
}
