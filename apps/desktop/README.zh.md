...
当 Web Host 进程意外退出时（崩溃，或某个插件——例如第三方插件市场的「重启」——在监管层之下拆掉 Host），应用会**自动重新拉起 Host 并重建窗口**，因此插件触发的重启会让应用回来，而不是停在退出状态。重拉是有界的：当 Host 每次都启动失败时，应用会放弃并退出，而不是死循环。显式退出（托盘 → 退出 / Cmd+Q）依旧不会自动重启。
