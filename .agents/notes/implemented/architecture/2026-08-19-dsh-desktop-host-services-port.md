...
- Startup-health reconciliation of a pending install-recovery record is owned by apps/desktop main.ts: each packaged generation claims the WAL, an awaiting-restart install is confirmed after the Host answers its first HTTP probe (then cleared) and rolled back when the Host exits or the probe fails; interrupted installs roll back at claim time. Unlike upstream's renderer-boot monitor, the thin Electron shell uses Host HTTP reachability as the health signal.
...
- Packaging contract (found in the first DMG smoke): the Electron main bundle must inline workspace packages (`tsdown.config.ts` sets `noExternal` for `@deepseek-ai/*`) — electron-builder stages only package folders into app.asar without resolving their peers, so an external import of dsh-desktop-host fails at startup on the missing cordis peer.
- First-run profiles created before the market entered `PROFILE_TEMPLATES.web` keep an older bundle list (upstream `dshmarket`, which the supply-chain minimumReleaseAge policy also rejects); existing `~/.dsh` profiles need the `@deepseek-ai/dsh-community-market` row added once for the market UI to appear.
