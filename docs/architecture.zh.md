...
[`dsh-base`](../packages/bundle/base/README.md) 是每个 profile 的第一层：模型适配器、工具、持久化、沙箱与审批策略、设置、凭据、遥测。[`dsh-web-app`](../packages/bundle/web-app/README.md) 增加浏览器应用；[`dsh-headless`](../packages/bundle/headless/README.md) 增加一次性运行器，且完全不带服务器。`web` profile 还随附 `@deepseek-ai/dsh-community-market`——内置的可视化插件市场：它把该插件列入 `dsh.profile.bundles`，并插入 `community-market` 主机条目及其浏览器客户端 bundle。独立的 `dshmarket` 仍可通过 `dsh plugin --profile web add dshmarket` 按需安装。
