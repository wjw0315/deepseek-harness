...
flowchart LR
  ...
  svc_desktopPnpm["ctx.desktopPnpm<br/>Desktop managed package manager"]
  pkg_community_market["community-market"]
  ...
  pkg_desktop_host --> svc_desktopPnpm
  ...
  svc_desktopPnpm --> pkg_community_market
...
| `ctx.desktopPnpm` | `core` | [`desktop-host`](../packages/host/desktop-host) | - | [`community-market`](../packages/market/community-market) | - | Runs one packaged-pnpm operation at a time against the active desktop profile behind a launcher-provided runtime; the community market is its only harness consumer. |
