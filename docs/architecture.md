...
[`dsh-base`](../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`dsh-web-app`](../packages/bundle/web-app/README.md) adds the browser application; [`dsh-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server at all. The `web` profile also ships `@deepseek-ai/dsh-community-market`, the in-box visual plugin market: it lists the plugin in `dsh.profile.bundles` and inserts the `community-market` host row plus its browser client bundle. The separately-published `dshmarket` remains installable on demand via `dsh plugin --profile web add dshmarket`.
